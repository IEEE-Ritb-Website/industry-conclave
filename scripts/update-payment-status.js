...
async function updatePaymentStatus() {
  ...
  try {
    ...
    console.log(`Found ${matchingDocs.length} documents with paymentScreenshot but isPaymentCompleted: false`);
    
    // Show sample matching documents
    if (matchingDocs.length > 0) {
      console.log('Sample matching documents:');
      matchingDocs.slice(0, 3).forEach((doc, index) => {
        console.log(`${index + 1}. Email: ${doc.email}, PaymentScreenshot: ${doc.paymentScreenshot ? 'exists' : 'missing'}, isPaymentCompleted: ${doc.isPaymentCompleted}`);
      });
    }
